	...
	public ResponseEntity<CategoryResponseRest> save(Category category) {
		...
		try {
			...
			if(categorySaved != null) {
				...
				response.getCategoryResponse().setCategory(list);
				response.setMetadata(Constantes.METADATA_OK, "00", "Categoria guardada");
			} else {
				response.setMetadata(Constantes.METADATA_NOK, "-1", "Categoria no guardada");
				return new ResponseEntity<>(response, HttpStatus.BAD_REQUEST);
			}
			
		} catch (Exception e) {
			response.setMetadata(Constantes.METADATA_NOK, "-1", "Error al grabar categoria");
			e.getStackTrace();
			return new ResponseEntity<>(response, HttpStatus.INTERNAL_SERVER_ERROR);
		}
